...
function Validate(event) {
  ...
  if (data.description_ar && !arabicChars.test(data.description_ar)) {
    return NotValid('*Only use arabic characters for arabic event description');
  }

  // Check if the user has selected a place
  if (!data.place) {
    NotValid('*Please select a location/place');
  } else {
    errorBox.innerText = '';
  }
  ...
  AddEventForm &&
    (data._method = 'post') &&
    axios.post(AddEventForm.action, data).then(function(res) {
      window.location = res.data.redirectUrl;
    });

  EditEventForm &&
    (data._method = 'put') &&
    axios.post(EditEventForm.action, data).then(function(res) {
      window.location = res.data.redirectUrl;
    });
}
AddEventForm && AddEventForm.addEventListener('submit', Validate);

EditEventForm && EditEventForm.addEventListener('submit', Validate);
